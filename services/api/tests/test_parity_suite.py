...
def test_reference_suite_parity_check_reports_full_accuracy() -> None:
    ...
    canonical = fixtures["reference_mumbai_1999_07_04_1222_v1"]
    assert canonical["compared_field_count"] >= 67
    assert canonical["verified_reference"] is True
    assert canonical["reference_status"] == "reference_captured"


def test_reference_suite_can_include_unverified_seed_cases() -> None:
    client = TestClient(app)
    response = client.get("/v1/engine/parity/reference-suite?include_unverified=true")
