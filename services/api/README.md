...
- `GET /v1/engine/parity/reference-suite` (strict parity on verified Reference fixtures)
...
## Accuracy Lock (Reference Suite)

Use this endpoint to validate all Reference fixtures in one run and get aggregated
accuracy:

```bash
curl "http://127.0.0.1:8000/v1/engine/parity/reference-suite"
```

By default this endpoint runs only **verified** references (`reference_captured` and
`reference_archived_export`), which is what CI gate enforcement uses.
