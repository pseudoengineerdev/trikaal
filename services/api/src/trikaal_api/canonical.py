...
REFERENCE_FIXTURE_DIR = WORKSPACE_ROOT / "libs" / "reference-fixtures" / "fixtures" / "reference"
...
def load_canonical_fixture() -> ReferenceFixture:
    fixture_path = REFERENCE_FIXTURE_DIR / "1999-07-04_mumbai_1222.json"
    return ReferenceFixture.model_validate(json.loads(fixture_path.read_text(encoding="utf-8")))


def load_reference_fixtures() -> list[ReferenceFixture]:
    fixtures: list[ReferenceFixture] = []
    for fixture_path in sorted(REFERENCE_FIXTURE_DIR.glob("*.json")):
        fixtures.append(
