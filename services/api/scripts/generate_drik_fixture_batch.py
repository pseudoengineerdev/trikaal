...
FIXTURE_DIR = WORKSPACE_ROOT / "libs" / "reference-fixtures" / "fixtures" / "drik"
ADMIN1_CODES_PATH = WORKSPACE_ROOT / "services" / "api" / "src" / "trikaal_api" / "data" / "admin1CodesASCII.txt"
OFFLINE_CAPTURE_DIR = Path("/tmp/drik_batch")
...
class CaptureCase:
    ...
    @property
    def drik_date(self) -> str:
        year, month, day = self.local_date.split("-")
        return f"{day}/{month}/{year}"

    @property
    def drik_time(self) -> str:
...
def main() -> None:
    ...
    for case in CASES:
        ...
        city_name = city["name"]
        page_html = load_drik_page_html(case=case, city_name=city_name)
        ...
        payload = {
            ...
            "source": "drik_panchang_search_capture",
            "captured_at_utc": captured_at_utc,
            "reference_status": "drik_captured",
            "birth_input": {
                "local_date": case.local_date,
                "local_time": case.local_time,
...
def fetch_drik_page_html(case: CaptureCase) -> str:
    url = (
        "https://www.drikpanchang.com/planet/position/planetary-positions-sidereal.html"
        f"?geoname-id={case.geoname_id}&date={case.drik_date}&time={case.drik_time}&lang=en"
    )
...
def load_drik_page_html(*, case: CaptureCase, city_name: str) -> str:
    ...
    return fetch_drik_page_html(case)


def parse_timezone(html: str) -> str:
    match = TIMEZONE_PATTERN.search(html)
    if not match:
        raise ValueError("Unable to parse timezone from Drik page source.")
...
def build_fixture_identity(city_name: str, local_date: str, local_time: str) -> tuple[str, Path]:
    ...
    fixture_id = f"drik_{city_slug}_{year}_{month}_{day}_{time_digits}_v1"
