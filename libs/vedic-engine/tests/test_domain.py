...
def test_default_profile_is_vedic_drik_lahiri() -> None:
    profile = CalculationProfile()

    assert profile.profile_id == "vedic_drik_lahiri_v1"
    assert profile.zodiac_system == "sidereal"
    assert profile.ayanamsha == "lahiri_chitrapaksha"
    assert profile.calculation_method == "drik_ganita"
...
def test_chart_snapshot_embeds_birth_event_and_profile() -> None:
    ...
    snapshot = ChartSnapshot(
        birth_event=event,
        profile=profile,
        metadata={"source": "drik_panchang"},
    )

    assert snapshot.birth_event.place_label == "Mumbai, India"
    assert snapshot.profile.profile_id == "vedic_drik_lahiri_v1"
    assert snapshot.metadata["source"] == "drik_panchang"
